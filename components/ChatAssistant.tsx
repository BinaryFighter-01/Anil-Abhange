...
const ChatAssistant: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      // FIX: Use import.meta.env.VITE_API_KEY for Vite local development
      const apiKey = import.meta.env.VITE_API_KEY;

      if (!apiKey) {
        throw new Error("VITE_API_KEY is missing. Please check your .env file.");
      }
      ...
      if (error instanceof Error) {
        if (error.message.includes("VITE_API_KEY is missing")) {
            errorMessage = "Configuration Error: VITE_API_KEY not found in .env file.";
        } else if (error.message.includes("API key not valid")) {
            errorMessage = "Authentication Error: The API Key in .env is invalid.";
        }
      }
